...
inline int32_t post_proc_rt_init()
{
  int32_t res = 0;
  ...
  std::cout << "Setting mem init: "  << proc_rt_info.is_rt <<": " << proc_is_rt() << std::endl;
...
///  if (proc_is_rt()) {
    ...
    free(buf);
    std::cout << "RT post init was successfull: " << "\n";
///  }
  return (0);
}
...
/// \return 0 on success, throw an exception on error
inline int32_t pre_proc_rt_init(const uint32_t cpu_bit_mask_in, const int32_t prio)
{
  int32_t res = 0;
  ...
  if (proc_rt_info.is_rt) {
    ...
    std::cout << "RT pre init was successfull: " << "\n";
  }
