...
public:
   ...
   * \tparam CycloneDDSQos The type of the QOS setting, for example data reader or data writer QOS.
...
private:
  ...
  {
    if (m_topic == 0) {
      m_topic = dds_create_topic (m_participant, Topic::CycloneDDSDesc(), Topic::topic_name().c_str(), nullptr, nullptr);
      if (m_topic < 0) {
